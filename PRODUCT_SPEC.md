...
- Student view provides Groups / Today / Pick a student navigation, a compact saved-arrangement selector, and Rebuild groups using that arrangement's existing settings. These limited projection controls are allowed; recipe editing, skill values, and relationship details remain in the teacher editor. The selector may show saved arrangement names. Switching pages preserves the reveal and fullscreen; switching arrangements or rebuilding covers groups until the next reveal. Rebuild is saved and undoable from the teacher editor. Today always uses the selected day and its own arrangement. Leaving Today pauses its timer and stops music; returning restores the paused round, reveal, and name visibility.
- **Pick a student** draws one student uniformly at random from the current class’s Here roster, including students who have not been placed in groups. Pick again makes a fresh independent draw, so repeats are possible. The chosen name fills a large card with a confetti reveal and the shared sound/mute setting; reduced motion reveals instantly. It does not change attendance, groups, starters, or rotations. The last pick remains while navigating within student view and clears on leaving it. Teacher-only student fields never enter this screen.
...
- The shared, modular reveal system offers Balloon lift-off, Rainbow zipper, Dragon hiccup, Cat & yarn, Saturn V · Moon landing, Black-hole whoosh, Fairy dust, Ocean wave, and Confetti party. A Surprise me option avoids the preceding effect. The original effects finish within 3.8 seconds; the longer illustrated stories take 6.8 seconds for the cat and 8.8 seconds for the Moon mission. Group and day screens use the same registry and playback controls, with teacher controls visible and usable during playback.
- A teacher can turn off effects. The display also honors reduced-motion preferences, revealing immediately without motion.
- Only present, placed students appear in group reveals. Empty groups are omitted. Larger arrangements and pairs wrap into additional rows.
