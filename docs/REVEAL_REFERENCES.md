...
The near foreleg draws over the chest with a soft shoulder blend, keeping the elbow and forearm visible during the raised-paw gesture. Its skin contour trims self-intersecting inside joins before rounding, so a tight elbow bend cannot produce a closed loop. Leg fur matches the body, and each limb paints once. This drawing-only correction was reviewed across six stride phases, six fold/reach poses, and six return poses, with an enlarged comparison at 3.00 seconds and the actual scene at 960×680. Bone lengths, paw contact, and performance timing are unchanged.
