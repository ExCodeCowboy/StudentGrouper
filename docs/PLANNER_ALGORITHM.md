...
4. The largest number of missing activities for any learner (fair coverage).
5. Total missing learner/activity visits.
6. Missing once-only learner/activity visits among equally good coverage plans.
7. Premature repeats: for each repeated learner visit, count the other eligible activities the learner had not yet visited before that round.
8. Consecutive repeats within a day.
9. Concentration of repeats within a day, measured by pairs of visits to the same activity.
10. Concentration of repeats across the block, using the same pair count.
11. Changes to existing editable assignments, to avoid needless reshuffling on repeated builds.
